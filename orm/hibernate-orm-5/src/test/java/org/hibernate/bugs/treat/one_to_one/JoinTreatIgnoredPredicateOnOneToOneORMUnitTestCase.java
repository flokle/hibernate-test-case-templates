...
class JoinTreatIgnoredPredicateOnOneToOneORMUnitTestCase {
	...
	void oneToOneExpectingNoResultTest(SessionFactoryScope scope) throws Exception {
		...
		scope.inTransaction(session -> {
			...
			String stmnt = "SELECT cat FROM Shelter s JOIN TREAT(s.animal as CatAnimal) cat ON cat.lives = :lives WHERE s.id = :shelterId";

			TypedQuery<AbstractAnimal> query = session.createQuery(stmnt, AbstractAnimal.class);
